...
func main() {
	...
	// Create Code Specialist first (child agent) - focuses on code analysis
	codeSpecialist, err := client.CreateAgent(ctx, &agentpg.AgentDefinition{
...
Always use your tools to back up your analysis.`,
	...
	if err != nil {
		log.Fatalf("Failed to create code-specialist agent: %v", err)
	}

	// Create Data Specialist (child agent) - focuses on data analysis
	dataSpecialist, err := client.CreateAgent(ctx, &agentpg.AgentDefinition{
...
Always query the data before providing insights.`,
	...
	if err != nil {
		log.Fatalf("Failed to create data-specialist agent: %v", err)
	}

	// Create Research Specialist (child agent) - focuses on knowledge search
...
Use your search tool to find relevant information.`,
	...
	// Create Orchestrator agent (parent) - no direct tools, delegates to specialists
...
Synthesize their responses into a cohesive answer for the user.`,
		// Orchestrator delegates to specialist agents (created as agent tools)
